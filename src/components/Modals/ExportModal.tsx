...
import { HighEndRenderAnimationModal } from './HighEndRenderAnimationModal';
...
export const ExportModal: React.FC<ExportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-xs flex items-center justify-center z-50 select-none p-4">
      ...
      {/* High-End 3D Multi-Layer Composition Animation & Studio */}
      <HighEndRenderAnimationModal
        ...
        jobConfig={{
          title: `Master-Render Pipeline (${format})`,
          operationType: format === 'WAV' ? 'EXPORT_WAV' : 'EXPORT_XML',
          positionSec: 0,
          durationSec: track.duration || 300,
          clipName: clips[0]?.name || 'Intro & Stabs',
          targetBpm: track.bpm,
          targetKey: track.key,
          fxName: 'True-Peak Brickwall Limiter & Master Re-clocking',
          outputFormat: format,
        }}
        onApplyRender={triggerActualDownload}
        onDownloadWav={triggerActualDownload}
      />
    </div>
  );
};
