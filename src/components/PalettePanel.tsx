...
interface PalettePanelProps {
  ...
  targetBpm?: number;
  targetKey?: string;
  onInsertClip?: (clip: PaletteClip) => void;
  onReplaceClip?: (clip: PaletteClip) => void;
  onOverdubClip?: (clip: PaletteClip) => void;
  onTriggerRenderAnimation?: () => void;
  onDuplicateClip?: (clipId: string) => void;
  onRenameClip?: (clipId: string, newName: string) => void;
  onDownloadClipWav?: (clip: PaletteClip) => void;
}
    ...
    <div className="w-72 bg-[#0e0f13] border-l border-[#1c1e25] flex flex-col select-none relative z-30 flex-shrink-0">
          ...
          clips.map((clip) => {
            ...
            return (
              ...
              >
                ...
                {/* Interactive Unfolded Clip Action Dock (Appears when clip is selected) */}
                <AnimatePresence>
                  {isSelected && (
                    <motion.div
                      ...
                      className="overflow-hidden border-t border-[#232838] bg-[#11141c] p-2 flex flex-col space-y-1.5"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <div className="text-[9.5px] font-mono text-neutral-400 flex justify-between items-center">
                        <span>AKTIONEN FÜR CLIP:</span>
                        <span className="text-[#00a2ff]">{clip.beats}B ({clip.duration.toFixed(2)}s)</span>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-1 text-[10px]">
                        {onInsertClip && (
                          <button
                            ...
                            className="px-2 py-1 rounded-xs bg-[#171b26] hover:bg-[#0088ff] hover:text-white text-neutral-200 flex items-center space-x-1 transition-colors"
                            title="Clip am Playhead einfügen"
                          >
                        ...
                        {onReplaceClip && (
                          <button
                            onClick={() => onReplaceClip(clip)}
                            className="px-2 py-1 rounded-xs bg-[#241a0e] hover:bg-[#ff9500] hover:text-black text-[#ff9500] border border-[#443015] flex items-center space-x-1 transition-colors"
                            title="Selektion durch diesen Clip ersetzen"
                          >
                            <Repeat size={11} />
                            <span>Replace</span>
                          </button>
                        ...
                        {onOverdubClip && (
                          <button
                            onClick={() => onOverdubClip(clip)}
                            className="px-2 py-1 rounded-xs bg-[#0c2418] hover:bg-[#00ff9d] hover:text-black text-[#00ff9d] border border-[#17482d] flex items-center space-x-1 transition-colors"
                            title="Signal 1 + Signal 2 zusammensetzen (Overdub)"
                          >
                            <Layers size={11} />
                            <span>Overdub</span>
                          </button>
                        ...
                        {onTriggerRenderAnimation && (
                          <button
                            ...
                            className="px-2 py-1 rounded-xs bg-[#152338] hover:bg-[#0088ff] hover:text-white text-[#00e5ff] border border-[#23426e] flex items-center space-x-1 transition-colors font-semibold"
                            title="Render-Animation mit diesem Clip ausführen"
                          >
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-[#1d2230]">
                        {onExpandToDeckView && (
                          <button
                            onClick={onExpandToDeckView}
                            className="text-[9.5px] text-[#00a2ff] hover:underline flex items-center space-x-1"
                            title="Clip in Deck B laden & analysieren"
                          >
                            <Maximize2 size={10} />
                            <span>In Deck B laden</span>
                          </button>
                        )}
                        <button
                          onClick={() => onDeleteClip(clip.id)}
                          className="text-[9.5px] text-[#ff5555] hover:text-[#ff3b30] flex items-center space-x-1 ml-auto"
                          title="Clip aus Palette entfernen"
                        >
                          <Trash2 size={10} />
                          <span>Löschen</span>
                        </button>
      ...
      {contextMenu && (
        <ClipContextMenu
          ...
          isPlayingPreview={playingClipId === contextMenu.clip.id}
          onTogglePreview={() => {
            if (playingClipId === contextMenu.clip.id) {
              audioEngine.stop();
              setPlayingClipId(null);
            } else if (contextMenu.clip.audioBuffer) {
              audioEngine.play(contextMenu.clip.audioBuffer, 0, false);
              setPlayingClipId(contextMenu.clip.id);
            }
          }}
          onInsertToDeckA={() => onInsertClip?.(contextMenu.clip)}
          onReplaceInDeckA={() => onReplaceClip?.(contextMenu.clip)}
          onOverdubInDeckA={() => onOverdubClip?.(contextMenu.clip)}
          onOpenRenderStudio={onTriggerRenderAnimation}
          onOpenInDeckB={() => {
            onSelectClip(contextMenu.clip);
            onExpandToDeckView?.();
          }}
          onRename={(newName) => onRenameClip?.(contextMenu.clip.id, newName)}
          onDuplicate={() => onDuplicateClip?.(contextMenu.clip.id)}
          onDownloadWav={() => onDownloadClipWav?.(contextMenu.clip)}
          onDelete={() => onDeleteClip(contextMenu.clip.id)}
          hasDeckASelection={hasSelection}
        />
      )}
    </div>
  );
