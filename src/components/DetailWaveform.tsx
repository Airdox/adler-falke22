  ...
  return (
    ...
    >
      ...
      <div className="flex-1 h-full relative overflow-hidden bg-[#0a0b0d]">
        ...
        {/* Interactive Floating Contextual Tool HUD (Unfolds when selection is active) */}
        <AnimatePresence>
          {selection && (
            <motion.div
              ...
              className="absolute top-3 left-1/2 -translate-x-1/2 bg-[#0e1118]/95 backdrop-blur-md border border-[#2b334a] rounded-xs shadow-2xl p-1.5 flex items-center space-x-1.5 z-40 text-xs select-none"
            >
              {/* Telemetry Badge */}
              <div className="px-2 py-0.5 rounded-xs bg-[#0088ff]/20 text-[#00e5ff] font-mono text-[10.5px] font-bold border border-[#0088ff]/40 mr-1 flex items-center space-x-1">
                <Sliders size={11} />
                <span>{Math.round(selection.beatsCount)} BEATS</span>
                <span className="text-neutral-400">({selection.duration.toFixed(2)}s)</span>
              </div>

              {/* Add to Palette (Clone) */}
              <button
                onClick={() => onAddToPalette(selection.start, selection.end)}
                className="px-2 py-1 rounded-xs bg-[#161a24] hover:bg-[#0088ff] hover:text-white text-neutral-200 text-[11px] font-medium flex items-center space-x-1 transition-colors"
                title="Auswahl als Clip in die Palette speichern"
              >
                <PlusSquare size={12} className="text-[#00a2ff]" />
                <span>Palette</span>
              </button>
              ...
              <button
                onClick={onCopy}
                className="px-2 py-1 rounded-xs bg-[#161a24] hover:bg-[#252c3e] text-neutral-200 text-[11px] font-medium flex items-center space-x-1 transition-colors"
              ...
              <button
                onClick={onCut}
                className="px-2 py-1 rounded-xs bg-[#161a24] hover:bg-[#252c3e] text-[#ff9500] text-[11px] font-medium flex items-center space-x-1 transition-colors"
              ...
              <button
                onClick={onReplace}
                className="px-2 py-1 rounded-xs bg-[#241a0e] hover:bg-[#ff9500] hover:text-black text-[#ff9500] border border-[#443015] text-[11px] font-medium flex items-center space-x-1 transition-colors"
                title="Auswahl durch Clip/Clipboard ersetzen"
              >
                <Repeat size={12} />
                <span>Replace</span>
              </button>

              {/* Overdub (Signal 1 ⊕ Signal 2) */}
              <button
                onClick={onOverdub}
                className="px-2 py-1 rounded-xs bg-[#0a2014] hover:bg-[#00ff9d] hover:text-black text-[#00ff9d] border border-[#144228] text-[11px] font-medium flex items-center space-x-1 transition-colors"
                title="Signal 1 und Signal 2 zusammensetzen (Overdub)"
              >
                <Layers size={12} />
                <span>Overdub (1+2=3)</span>
              </button>

              {/* Trigger High-End Render Studio */}
              {onTriggerRenderAnimation && (
                <button
                  onClick={onTriggerRenderAnimation}
                  className="px-2.5 py-1 rounded-xs bg-[#122238] hover:bg-[#0088ff] hover:text-white text-[#00e5ff] border border-[#23426e] text-[11px] font-bold flex items-center space-x-1 transition-colors"
              ...
              <button
                onClick={onDelete}
                className="px-2 py-1 rounded-xs bg-[#241215] hover:bg-[#ff453a] hover:text-white text-[#ff5555] border border-[#441a20] text-[11px] font-medium flex items-center space-x-1 transition-colors"
                title="Löschen & Schließen"
              >
                <Trash2 size={12} />
                <span>Löschen</span>
              </button>

              <div className="w-px h-4 bg-[#2b334a] mx-0.5" />

              {/* Cancel / Deselect */}
              <button
                onClick={() => onSelect(null)}
                className="p-1 rounded-xs hover:bg-[#2b334a] text-neutral-400 hover:text-white transition-colors"
