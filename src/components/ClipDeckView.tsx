...
export const ClipDeckView: React.FC<ClipDeckViewProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#0b0c0f] border-t border-[#1c1e26] select-none z-30 overflow-hidden">
      ...
      {/* Context Menu for Clip & Waveform */}
      {contextMenu && (
        <ClipContextMenu
          isOpen={true}
          ...
          isPlayingPreview={isPlaying && activeClip?.id === contextMenu.clip.id}
          onTogglePreview={() => {
            if (activeClip?.id === contextMenu.clip.id) {
              if (isPlaying) {
                audioEngine.stop();
                setIsPlaying(false);
              } else if (contextMenu.clip.audioBuffer) {
                audioEngine.play(contextMenu.clip.audioBuffer, currentTime, loopActive);
                setIsPlaying(true);
              }
            } else {
              onSelectClip(contextMenu.clip);
              if (contextMenu.clip.audioBuffer) {
                audioEngine.play(contextMenu.clip.audioBuffer, 0, false);
                setIsPlaying(true);
              }
            }
          }}
          onInsertToDeckA={() => onInsertClipToDeckA(contextMenu.clip)}
          onReplaceInDeckA={() => onReplaceDeckAWithClip(contextMenu.clip)}
          onOverdubInDeckA={() => onOverdubDeckAWithClip(contextMenu.clip)}
          ...
          hasDeckASelection={hasSelectionInDeckA}
          timeAtClick={contextMenu.timeAtClick}
          onSeekToTime={(time) => {
            setCurrentTime(time);
            if (isPlaying && activeClip?.audioBuffer) {
              audioEngine.play(activeClip.audioBuffer, time, loopActive);
            }
          }}
        />
      )}
