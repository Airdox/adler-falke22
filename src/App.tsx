...
import { HighEndRenderAnimationModal, RenderJobConfig } from './components/Modals/HighEndRenderAnimationModal';
...
export default function App() {
  ...
  // High-End Render Animation Studio State
  const [renderModalOpen, setRenderModalOpen] = useState<boolean>(false);
  const [renderJobConfig, setRenderJobConfig] = useState<RenderJobConfig | undefined>(undefined);
  ...
  // High-End Render Animation Studio trigger handler
  const handleOpenRenderStudio = useCallback((customConfig?: Partial<RenderJobConfig>) => {
    if (!activeTrack) return;
    const pos = selection ? selection.start : currentTime;
    const dur = selection ? selection.duration : 7.38;
    const selClip = paletteClips.find((c) => c.id === selectedClipId) || paletteClips[0] || null;

    setRenderJobConfig({
      title: customConfig?.title || 'Signal 1 ⊕ Signal 2 Zusammensetzung (Overdub/Render)',
      operationType: customConfig?.operationType || (selection ? 'OVERDUB' : 'EXPORT_WAV'),
      positionSec: customConfig?.positionSec ?? pos,
      durationSec: customConfig?.durationSec ?? dur,
      clipName: customConfig?.clipName || selClip?.name || 'Vocals & Synth Stabs 8B',
      clipBpm: customConfig?.clipBpm || selClip?.bpm || activeTrack.bpm,
      clipKey: customConfig?.clipKey || selClip?.key || activeTrack.key,
      targetBpm: activeTrack.bpm,
      targetKey: activeTrack.key,
      fxName: customConfig?.fxName || 'Resonanz Filter Sweep & 1/2 Beat Echo',
      outputFormat: customConfig?.outputFormat || 'WAV',
    });
    setRenderModalOpen(true);
  }, [activeTrack, selection, currentTime, paletteClips, selectedClipId]);
  ...
  // Replace selection (or from currentTime) in Deck A with Clip (with Tempo & Harmonic Pitch Adaptation)
  const handleReplaceDeckAWithClip = (clip: PaletteClip) => {
    ...
    const targetStart = selection ? selection.start : currentTime;
    const targetDuration = selection ? selection.duration : adapted.newDuration;
    const barsCount = selection ? selection.barsCount : (targetDuration / (60 / activeTrack.bpm)) / 4;
    const beatsCount = selection ? selection.beatsCount : targetDuration / (60 / activeTrack.bpm);
    ...
    const replaceSeg: EditSegment = {
      ...
      trackId: activeTrack.id,
      sourceStart: 0,
      sourceEnd: Math.min(adapted.newDuration, targetDuration),
      projectStart: targetStart,
    ...
    showOperationFeedback({
      title: 'Deck A ersetzt (Replace mit Clip)',
      operationType: 'REPLACE',
      description: `Bereich (${targetDuration.toFixed(3)}s / ${barsCount.toFixed(1)} Takte ab ${targetStart.toFixed(2)}s) durch Clip "${clip.name}" ersetzt. Tempo angepasst: ${clip.bpm.toFixed(1)} ➔ ${activeTrack.bpm.toFixed(1)} BPM (${adapted.tempoRatio.toFixed(3)}×). ${
        adapted.semitonesShifted !== 0
      ...
      timeRangeSec: { start: targetStart, end: targetStart + targetDuration, duration: targetDuration },
      barsCount: barsCount,
  ...
  // Overdub in Deck A with Clip (with Tempo & Harmonic Pitch Adaptation)
  const handleOverdubDeckAWithClip = (clip: PaletteClip) => {
    ...
    showOperationFeedback({
      ...
      description: `Clip "${clip.name}" über Deck A gemischt (${targetDuration.toFixed(3)}s / ${barsCount.toFixed(1)} Takte ab ${targetStart.toFixed(2)}s). Tempo angepasst: ${clip.bpm.toFixed(1)} ➔ ${activeTrack.bpm.toFixed(1)} BPM (${adapted.tempoRatio.toFixed(3)}×). ${
        adapted.semitonesShifted !== 0
          ? `Tonhöhe: um ${adapted.semitonesShifted > 0 ? '+' : ''}${adapted.semitonesShifted} Halbtöne angepasst (${adapted.harmonicRelation}).`
          : 'Tonhöhe: Harmonisch kompatibel.'
      }`,
  ...
  // Clip Management Handlers (Context Menu & Palette actions)
  ...
  const handleRenameClip = useCallback((id: string, newName: string) => {
  ...
  const handleDownloadClipWav = useCallback((clip: PaletteClip) => {
    if (!clip.audioBuffer) return;
    const wavBlob = audioEngine.exportToWavBlob(clip.audioBuffer);
    const url = URL.createObjectURL(wavBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${clip.name.replace(/[^a-zA-Z0-9_-]/g, '_')}.wav`;
    a.click();
  ...
  return (
    <div className="w-screen h-screen bg-[#0a0b0d] flex flex-col overflow-hidden select-none text-neutral-200">
      ...
      <MenuBar
        ...
        onOpenRenderInspector={() => handleOpenRenderStudio()}
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <DetailWaveform
          ...
          onTriggerRenderAnimation={() => handleOpenRenderStudio()}
        />
        ...
        {paletteViewMode === 'SIDEBAR' && (
          <PalettePanel
            ...
            targetBpm={activeTrack?.bpm}
            targetKey={activeTrack?.key}
            onInsertClip={handleInsertClipToDeckA}
            onReplaceClip={handleReplaceDeckAWithClip}
            onOverdubClip={handleOverdubDeckAWithClip}
            onTriggerRenderAnimation={() => handleOpenRenderStudio({ operationType: 'OVERDUB' })}
            onDuplicateClip={handleDuplicateClip}
            onRenameClip={handleRenameClip}
            onDownloadClipWav={handleDownloadClipWav}
          />
        )}
      </div>
      ...
      {/* 6. Lower Action Block: Collapsible & Interactive Unfolding Tools */}
      <BottomControlBlock
        selection={selection}
      ...
      {/* High-End 3D Multi-Schichten Render-Studio Modal (Signal 1 ⊕ 2 = 3) */}
      {activeTrack && (
        <HighEndRenderAnimationModal
          isOpen={renderModalOpen}
          onClose={() => setRenderModalOpen(false)}
          track={activeTrack}
          workingAudioBuffer={workingAudioBuffer}
          activeClip={paletteClips.find((c) => c.id === selectedClipId) || paletteClips[0] || null}
          ...
          onDownloadWav={() => {
            if (workingAudioBuffer) {
              const wavBlob = audioEngine.exportToWavBlob(workingAudioBuffer);
              const url = URL.createObjectURL(wavBlob);
              const a = document.createElement('a');
              a.href = url;
              a.download = `${activeTrack.title || 'Render'}_Master.wav`;
              a.click();
              URL.revokeObjectURL(url);
            }
          }}
